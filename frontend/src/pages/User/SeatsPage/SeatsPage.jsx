    ...
    return (
      <div className="min-h-screen bg-background flex items-center justify-center">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-on-surface-variant font-medium animate-pulse">Đang chuẩn bị phòng vé...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-background flex flex-col items-center justify-center gap-6">
        <div className="w-20 h-20 rounded-full bg-error/10 flex items-center justify-center">
          <span className="material-symbols-outlined text-error text-4xl">stadium</span>
        </div>
        <div className="text-center">
          <h2 className="text-2xl font-bold mb-2">Suất chiếu không khả dụng</h2>
          <p className="text-on-surface-variant font-medium">Xin lỗi, suất chiếu này đã kết thúc hoặc không tìm thấy thông tin.</p>
        </div>
        <button onClick={() => navigate(-1)} className="bg-surface-container px-6 py-3 rounded-xl font-bold hover:bg-surface-bright transition-colors">
          Quay lại
        </button>
      </div>
    );
                    ...
                    return (
                      <React.Fragment key={col}>
                        ...
                        <button
                          ...
                          `}
                        >
                          <span className={`${isSelected ? "text-on-secondary" : "text-on-surface-variant opacity-40"} text-[8px] font-bold`}>
                            {seatId}
                          </span>
                        </button>
                      </React.Fragment>
        ...
        <div className="mt-12 grid grid-cols-1 md:grid-cols-12 gap-8 pb-20 items-center">
          ...
          <div className="md:col-span-9">
            ...
            <div className="flex flex-wrap gap-4 mb-6">
              {movie.tags.map(tag => (
                <span key={tag} className="px-3 py-1 bg-surface-variant/40 rounded-full border border-outline-variant/20 text-xs font-semibold text-primary uppercase">{tag}</span>
              ))}
              <span className="px-3 py-1 bg-surface-variant/40 rounded-full border border-outline-variant/20 text-xs font-semibold text-primary">{showtime.format}</span>
              <span className="px-3 py-1 bg-surface-variant/40 rounded-full border border-outline-variant/20 text-xs font-semibold text-tertiary">{movie.rating} ★</span>
            </div>
        ...
        <div className="max-w-5xl mx-auto bg-surface-container/80 backdrop-blur-xl rounded-3xl p-6 shadow-[0_-10px_30px_rgba(0,0,0,0.5)] border border-white/5 flex flex-col md:flex-row justify-between items-center gap-6">
          <div className="grid grid-cols-2 md:grid-cols-3 gap-8 items-center w-full md:w-auto">
            <div className="flex flex-col">
              ...
              <span className="text-sm font-bold text-on-surface">{cinema.name} • {showtime.time}</span>
            </div>
